...
def main():
    ...
    st.markdown("<h4>Upload .fastq.gz files or zipped barcode folders with reference FASTA.</h4>", unsafe_allow_html=True)
    ...
    if process_btn:
        if not reference_file or (not fastq_files and not zip_files):
            st.error("❌ Please upload a reference file and either FASTQ or ZIP inputs.")
            return
...
def process_inputs(reference_file, fastq_files, zip_files):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for idx, fq_file in enumerate(input_files, 1):
            ...
            subprocess.run(f"LC_ALL=C samtools coverage {sorted_bam} > {coverage_tsv}", shell=True, executable="/bin/bash")

            with open(coverage_csv, "w") as out_csv, open(coverage_tsv) as f:
                out_csv.write("#rname,startpos,endpos,numreads\n")
                for line in f:
                    if line.startswith("#"): continue
                    parts = line.strip().split("\t")
                    if len(parts) >= 4:
                        out_csv.write(f"{parts[0]},{parts[1]},{parts[2]},{int(float(parts[3]))}\n")
